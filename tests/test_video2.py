...
if __name__ == "__main__":
    ...
    while True:
        ...
        ball_data = {
            "label": ball_detections[0]["label"],
            "confidence": ball_detections[0]["confidence"],
            "position": position
        }

        human_data = {
            "label": human_detections[0]["label"],
            "confidence": human_detections[0]["confidence"],
            "position": position
        }

        # Priny information
        print(f"🤟 Frame {current_frame_idx + 1}/{total_frames}")
        print(f"⚽️Ball detections:")
        pprint(pprint(ball_data))
        print(f"Position: {position}")

        print(f"👨🏻‍🔧Human detections:")
        pprint(pprint(human_data))
